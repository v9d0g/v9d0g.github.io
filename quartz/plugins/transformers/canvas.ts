              ...
              if (
                ...
                const scriptNode: Element = {
                  ...
                  children: [
                    {
                      type: "text",
                      value: `(function(){document.querySelectorAll('.quartz-canvas-placeholder').forEach(function(el){if(el.dataset.canvasReady)return;el.dataset.canvasReady='1';el.addEventListener('click',function(){var s=this.dataset.canvasSrc,m=document.createElement('div');m.className='quartz-canvas-modal';m.innerHTML='<div class="quartz-canvas-modal-backdrop"></div><div class="quartz-canvas-modal-content"><button class="quartz-canvas-modal-close" aria-label="关闭">&times;</button><iframe src="'+s+'" class="quartz-canvas-iframe" frameBorder="0" allow="fullscreen" title="Obsidian Canvas"></iframe></div>';var pb=document.body.style.overflow,ph=document.documentElement.style.overflow;document.body.style.overflow='hidden';document.documentElement.style.overflow='hidden';document.body.appendChild(m);function c(){document.body.style.overflow=pb;document.documentElement.style.overflow=ph;m.remove();}m.querySelector('.quartz-canvas-modal-close').addEventListener('click',c);m.querySelector('.quartz-canvas-modal-backdrop').addEventListener('click',c);m.addEventListener('wheel',function(e){e.preventDefault();},{passive:false});});});})();`,
                    },
                  ],
                }
