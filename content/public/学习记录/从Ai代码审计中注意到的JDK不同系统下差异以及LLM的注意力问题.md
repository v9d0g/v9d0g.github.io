...
自己把自己给想宕机了
## 优化
为了能够稳定的实现让ai能够挖掘类似的漏洞，如何把能力落地固化，让其具有可迁移的能力，是目前需要解决的问题

模型本质需要的还是prompt、有效的提示词来让模型注意力集中到代码和漏洞本身

这里的有效我认为是几个方面
- 1.有效的代码，纯代码，作为事实依据
- 2.有效的代码逻辑解释，真实的分支处理，条件判断前提、经过、结果
- 3.有效的相关漏洞知识，真实漏洞是什么，遇到某种场景之后的绕过方法有哪些

针对常规的系统漏洞（RCE、SQL注入等非权限、业务的逻辑漏洞）source->sink这条链是肯定存在的

有一个触发的点和一个具体问题存在的点，这两个点之间肯定也会存在某种直接或者间接的调用关系

所以针对`1.`我们是能通过各种静态代码分析的手段获取全部的代码信息，由于这里有静态工具从代码本身层面来分析，那么这个代码是具有强关联的事实依据

相当于这里是给模型提供了分析的清单，让模型关注某些特定的调用链，有点类似于人工代码审计的时候从sink反推是否有可达source

当然如果成本没有限制的情况下，遍历所有可达的路径（web系统中每个接口、组件框架中每个api或者可被开发者或者用户接触到的端点）也可以

而针对`2.`，我认为是需要让模型显式的回答他理解出来的逻辑，并且这个回答要一定程度上在整个对话的上下文中

能记录，可追溯，可反思；当然这里需要一定程度上的引导模型来思考和回答这类问题，我觉得使用Skill或者硬性的work flow可以实现

而`3.`依赖多个因素，模型本身的安全能力以及渐进式加载的小技巧prompt

针对上面的那个具体的漏洞，通过引入一套渐进式披露的prompt和每个分析阶段记录的显式回答，可复现概率就大了不少
...
但这种也**不是每次测试都能稳定复现**，使用ai进行代码审计还是存在一定的随机性，没有完全能够硬性约束的合理方式

过度约束会影响模型自身能力，例如想到一些人工会忽略的点

而过少约束会导致模型天马行空的操作，从而造成大量误报

而且在模型一周一个样，一个月一个大变的现在，做的这些很有可能只是整个大模型发展的中间态，不过考虑到各种因素（例如成本），适当的脚手架对较差的模型是具有提升的效果的
